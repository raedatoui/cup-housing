package com.cup.display {
	...
	public class IncomeChart extends BlockSprite {
		...
		public var minmfi:int = 0;
		public var mfiPercents:Array = [0, .3, .5, .8, 1, 1.2, 2.5];
		public var mfiIncomes:Array = [0, 23050, 38400, 61450, 91150, 192000];
		public var mfiDescription:String = 'The bar below shows the income categories that the government uses to decide who is eligible for different affordable housing programs. These categories are expressed as percentages of the “median family income.” The MFI for a family of four in Greater New York (which includes the City and some of its suburbs) is $76,800 per year';
		...
		protected var max:int = int(maxmfi * 1.1);
		...
		public function IncomeChart(w:Number = 0, h:Number = 0, mfiData:Object=null) {
			...
			this.mfiPercents = mfiData.mfiPercents2;
			this.popPercents = mfiData.mfiPercents;
			this.mfiTips = [mfiData.mfiTip0, mfiData.mfiTip1, mfiData.mfiTip2, mfiData.mfiTip3, mfiData.mfiTip4, mfiData.mfiTip5];

			var	chartCoeffs:Array = [];
			var maxPercent:Number = maxmfi / mfi;
			for(var i:int = 0; i < popPercents.length; i++)
				chartCoeffs.push(popPercents[i]);
			chartCoeffs.push(maxPercent * coeff);
			...
			chart = new LabelPropBlockSprite(this.populations, mfiLabels, mfiTips, mfiPopColors, chartCoeffs, (maxPercent * coeff), w, h + spillUp);
			addChild(chart);
			...
			chartLabels = new PropLabelSprite(20000, 500, mfi * maxPercent* coeff, mfi * .025 * maxPercent * coeff, 0, 4, w, h, RGB.white());
			chartLabels.y = h / 2;
...
//				display.x = curX;
		...
		public function realignColumns():void {
			this.chart.resize();
		}
